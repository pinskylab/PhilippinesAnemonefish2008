...
setwd("/Users/mpinsky/Documents/Stanford/Philippines/2008/Surveys/gps")
source("readGPX.R")
options(digits=22)
...
####### Add lat/long to survey data
	...
	latlong = read.csv("gps/track.concat2008-11-21.csv", row.names=1)
	...
	for(i in 1:len){
		...
		# Find the location records that match the date/time stamp (to nearest minute)
		latlongindex = which(latlong$month == month & latlong$day == day & latlong$hour == hour & latlong$min == min)
	
		# Calculate the mean lat/long for this minute
		data$lat[i] = mean(latlong$lat[latlongindex])
		data$lon[i] = mean(latlong$long[latlongindex])

		# Add the total number of fish (only for dominant spp)
		data$NumFish[i] = sum(c(!is.na(data$Size1[i]), !is.na(data$Size2[i]), !is.na(data$Size3[i]), !is.na(data$Size4[i]), !is.na(data$Size5[i])))
		if(!is.na(data$Size6[i]) & !(data$Size6[i] == '')){ # parse Size6 if needed
			data$NumFish[i] = data$NumFish[i] + length(strsplit(as.character(data$Size6[i]), split=',')[[1]])
		}

		# Add the size of fish (only for dominant spp)
		temp = c(data$Size1[i], data$Size2[i], data$Size3[i], data$Size4[i], data$Size5[i])
		temp = temp[!is.na(temp)]
		temp = paste(temp, collapse=',')
		if(!is.na(data$Size6[i]) & !(data$Size6[i] == '')){ # parse Size6 if needed
			temp = paste(temp, data$Size6[i], sep=',')
		}
		data$Sizes[i] = temp
	}
	...
	# Sort the data
	permut = order(data$SurveyNum, data$ObsTime)
	data = data[permut,]
	row.names(data) = 1:nrow(data)
	
	# Examine the data
	data[,c('SurveyNum', 'ObsTime', 'AnemSpp', 'Spp', 'NumFish', 'Sizes', 'lat', 'lon')]
	
	
	# Write out collection data
	write.csv(data, file=paste("GPSSurvey.anemlatlong", Sys.Date(), ".csv", sep=""))
