...
reefarea = data.frame(name = c("Philippines", "Study Area", "CebuLeyte", "Cebu", "Leyte", "Cebu Study Area", "Leyte Study Area"), ArcArea = c(1260, 79,47, 18.9, 28.5, 124*.15, 159*.15), PolArea = c(5238, 498, 121, 85, 36, NA, NA), PolPerimArea = c(2418, 182, 35.4, 27.5, 7.95, 183*.15, 53*.15), TotalAreaPolArea = c(6499, 576, 168, 104, 64, NA, NA), TotalAreaPolPerim = c(3713, 264, 83, 47, 37, 47, 32))  # numbers are from PhilsReefArea 090318.R with reef arcs 150m wide
...
polsurvs = match(polsurveys, surv$SurveyNum)
arcsurvs = match(arcsurveys, surv$SurveyNum)
mean(surv$densAPCLad[intersect(which(surv$Region=="Cebu"), c(polsurvs,arcsurvs))], na.rm=T) # mean density on reefs
mean(surv$densAPCLad[intersect(which(surv$Region=="Leyte"), c(polsurvs,arcsurvs))], na.rm=T) # mean density on reefs
mean(surv$densAPCLad[-c(polsurvs,arcsurvs)], na.rm=T) # mean density not on reefs
...
c("CebuLeyte per km", NumAPCL_polarea[3]/600, NumAPCL_polperim[3]/600)
c("Cebu per km", NumAPCL_polarea[4]/250, NumAPCL_polperim[4]/250)
c("Leyte per km", NumAPCL_polarea[5]/350, NumAPCL_polperim[5]/350)

# bootstrap estimate of se of fish/km (matching surveys to arcs or polygons)
...
km=350
np = length(p) # number of polygon abundance estimates
na = length(a) # number of arc abundance estimates
b = numeric(0) # to hold the bootstrap estimates of adults/km
c = numeric(0) # to hold the bootstrap estimates of adults
for(i in 1:10000){
...
# Adults: survey density * (arcs + polygon area) and converting back to adults/km
polsurvs = match(polsurveys, surv$SurveyNum)
arcsurvs = match(arcsurveys, surv$SurveyNum)
mean(surv$densAPCLad[intersect(which(surv$Region=="Cebu"), c(polsurvs,arcsurvs))], na.rm=T) # mean density on reefs Cebu
sd(surv$densAPCLad[intersect(which(surv$Region=="Cebu"), c(polsurvs,arcsurvs))], na.rm=T)/sqrt(length(intersect(which(surv$Region=="Cebu"), c(polsurvs,arcsurvs)))) # sd density on reefs Cebu
mean(surv$densAPCLad[intersect(which(surv$Region=="Leyte"), c(polsurvs,arcsurvs))], na.rm=T) # mean density on reefs Leyte
sd(surv$densAPCLad[intersect(which(surv$Region=="Leyte"), c(polsurvs,arcsurvs))], na.rm=T)/sqrt(length(intersect(which(surv$Region=="Leyte"), c(polsurvs,arcsurvs)))) # sd density on reefs Leyte

# vector of mean densities
dens = c(rep(mean(surv$densAPCLad[c(polsurvs,arcsurvs)], na.rm=T), 3), mean(surv$densAPCLad[intersect(which(surv$Region=="Cebu"), c(polsurvs,arcsurvs))], na.rm=T), mean(surv$densAPCLad[intersect(which(surv$Region=="Leyte"), c(polsurvs,arcsurvs))], na.rm=T), mean(surv$densAPCLad[intersect(which(surv$Region=="Cebu"), c(polsurvs,arcsurvs))], na.rm=T), mean(surv$densAPCLad[intersect(which(surv$Region=="Leyte"), c(polsurvs,arcsurvs))], na.rm=T)) # mean densities for Phils, study area, CebuLeyte, Cebu, Leyte, Cebu study area, Leyte study area 

# vector of SE of the mean
ses = c(rep(sd(surv$densAPCLad[c(polsurvs,arcsurvs)], na.rm=T)/sqrt(length(c(polsurvs,arcsurvs))), 3), sd(surv$densAPCLad[intersect(which(surv$Region=="Cebu"), c(polsurvs,arcsurvs))], na.rm=T)/sqrt(length(intersect(which(surv$Region=="Cebu"), c(polsurvs,arcsurvs)))), sd(surv$densAPCLad[intersect(which(surv$Region=="Leyte"), c(polsurvs,arcsurvs))], na.rm=T)/sqrt(length(intersect(which(surv$Region=="Leyte"), c(polsurvs,arcsurvs)))), sd(surv$densAPCLad[intersect(which(surv$Region=="Cebu"), c(polsurvs,arcsurvs))], na.rm=T)/sqrt(length(intersect(which(surv$Region=="Cebu"), c(polsurvs,arcsurvs)))), sd(surv$densAPCLad[intersect(which(surv$Region=="Leyte"), c(polsurvs,arcsurvs))], na.rm=T)/sqrt(length(intersect(which(surv$Region=="Leyte"), c(polsurvs,arcsurvs))))) # SE of mean densities for Phils, study area, CebuLeyte, Cebu, Leyte, Cebu study area, Leyte study area 

# dataframe of density calcs
out = data.frame(name=reefarea$name, ad = round(((reefarea$TotalAreaPolPerim)*dens)*1000^2), adse = round(((reefarea$TotalAreaPolPerim)*ses)*1000^2), km = c(NA, NA, 600, 250, 350, 252, 223)) # using only polygon perimeters (150m buffer), in km2
out$adkm = out$ad/out$km
out$adkmse = out$adse/out$km
format(out, big.mark=",", digits=5)

# t-tests of reef surveys vs. standard (random-site) surves
t.test(x = surv$densAPCLad[intersect(which(surv$Region=="Cebu"), c(polsurvs,arcsurvs))]*reefarea$TotalAreaPolPerim[4]*1000^2/250, y=surv$densAPCLad[surv$Region=="Cebu" & surv$RandomSite==1]*1000*150) # t.test vs. random surveys (all of Cebu)

t.test(x = surv$densAPCLad[intersect(which(surv$Region=="Leyte"), c(polsurvs,arcsurvs))]*reefarea$TotalAreaPolPerim[5]*1000^2/350, y=surv$densAPCLad[surv$Region=="Leyte" & surv$RandomSite==1]*1000*150) # all of Leyte

t.test(x = surv$densAPCLad[intersect(which(surv$Region=="Cebu"), c(polsurvs,arcsurvs))]*reefarea$TotalAreaPolPerim[6]*1000^2/252, y=surv$densAPCLad[surv$Region=="Cebu" & surv$RandomSite==1]*1000*150) # t.test vs. random surveys (Cebu study area)

t.test(x = surv$densAPCLad[intersect(which(surv$Region=="Leyte"), c(polsurvs,arcsurvs))]*reefarea$TotalAreaPolPerim[7]*1000^2/223, y=surv$densAPCLad[surv$Region=="Leyte" & surv$RandomSite==1]*1000*150) # all of Leyte
...
# FOR EVOL PAPER (SUBMISSION #2): A clarkii fish and adults/m2 for Cebu and Leyte, Random and Reef-associated Fish Surveys
# need arcsurveys and polsurveys from above ("Extrapolate density to population size")
a = surv[order(surv$lat),]
polsurvinds = match(polsurveys, a$SurveyNum)
arcsurvinds = match(arcsurveys, a$SurveyNum)
...
xlims = c(9.4, 11.4)
k = intersect(which(a$Region == "Cebu"), c(arcsurvinds, polsurvinds))
plot(a$lat[k], a$densAPCL[k], main = "Cebu", xlab = "Latitude (°)", ylab = "fish/m2", ylim= ylims, xlim=xlims, type="p", pch=4)
aggregate(a$densAPCL[k], by=list(a$SiteNum[k]), mean) # mean density for each reef site
mean(a$densAPCL[k]) # mean fish/m2 across all reef surveys
sd(a$densAPCL[k])/sqrt(length(k)) # se fish/m2 across reef sites
mean(a$densAPCLad[k]) # mean adults/m2 across all reef surveys
sd(a$densAPCLad[k])/sqrt(length(k)) # se adults/m2 across reef sites
k = a$Region == "Cebu" & a$RandomSite == 1
lines(a$lat[k], a$densAPCL[k], type="o", pch=20, lty=2) # plot fish/m2 from random surveys
lines(a$lat[k], a$densAPCLad[k], type="o", pch=20) # plot adults/m2 from random surveys

xlims = c(9.95, 11.1)
k = intersect(which(a$Region == "Leyte"), c(arcsurvinds, polsurvinds))
plot(a$lat[k], a$densAPCL[k], main = "Leyte", xlab = "Latitude (°)", ylab = "fish/m2", ylim= ylims, xlim=xlims, type="p", pch=4)
aggregate(a$densAPCL[k], by=list(a$SiteNum[k]), mean) # mean density for each reef site
mean(a$densAPCL[k]) # mean fish/m2 across all reef surveys
sd(a$densAPCL[k])/sqrt(length(k)) # se fish/m2 across reef sites
...
# All fish spp: Random sites: Adults/m2
	fishlist = c("PRBI", "APCL", "APOC", "APML", "APSA", "APPE", "APPY")
	quartz(width=10, height=8)
	par(mfrow=c(2,4))
	for(thisspp in fishlist){
		dens = paste("dens", thisspp, "ad", sep="")
		x = surv[surv$Region != "Danajon", c("Region", "RandomSite", dens)]
		x$Region = x$Region[,drop=TRUE]
		names(x) = c("Region", "RandomSite", "dens")
		boxplot(dens ~ Region, data=x, subset = x$RandomSite ==1, main = thisspp, ylab = "adults/m2", range=0, boxwex=0.7, staplewex=0.2, outwex = 0.5, lwd=1, col="dark grey")
	}

# All fish spp: All sites: Adults/m2
	fishlist = c("PRBI", "APCL", "APOC", "APML", "APSA", "APPE", "APPY")
	quartz(width=10, height=8)
	par(mfrow=c(2,4))
	for(thisspp in fishlist){
		...
		x$Region = x$Region[,drop=TRUE]
		names(x) = c("Region", "RandomSite", "dens")
		boxplot(dens ~ Region, data=x, main = thisspp, ylab = "adults/m2", range=0, boxwex=0.7, staplewex=0.2, outwex = 0.5, lwd=1, col="dark grey")
	}
