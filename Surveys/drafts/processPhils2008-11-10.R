...
names(collections) = c("SurveyNum", "Time", "Spp", "Size", "ID")
collections$lat = NA
collections$long = NA
...
# fix Survey #22 (Casay) samples taken after the GPS track: (insert ave lat/long)

# fix survey #38 (need to find problem, too)
...
# collections = read.csv("Collection2008-11-10.csv")
# surv = read.csv("GPSSurveys2008-11-10.csv")

surveys = unique(collections$SurveyNum)

for(i in surveys){
	k = which(collections$SurveyNum == i)
	numapcl = sum(collections$Spp[k] == "APCL")
	numprbi = sum(collections$Spp[k] == "PRBI")
which
}
