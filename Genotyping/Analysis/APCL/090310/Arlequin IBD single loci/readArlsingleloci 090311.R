setwd("C:/Documents and Settings/Mollie Manier/Desktop/Users/Malin/Philippines 2008/Analysis/090310/Arlequin IBD single loci")
...
for(i in 1:13){
	...
	title = strsplit(strsplit(names(fsts)[i], "CebuLeyt ")[[1]][2],".", fixed=T)[[1]][1]
	plot(cebugeo, cebu, main=title, xaxt="n", yaxt="s", pch=1, ylim = c(0,0.18))
...
row.names(out) = unlist(strsplit(unlist(strsplit(names(fsts), "CebuLeyt "))[seq(2,26,2)],".", fixed=T))[seq(1,25,2)]


write.csv(out, "MantelResults.csv")
