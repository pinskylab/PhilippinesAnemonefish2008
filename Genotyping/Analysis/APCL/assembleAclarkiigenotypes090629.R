...
# max num alleles per locus
numall = numeric(13)
alleles = vector("list", 13)
for(i in 1:length(cols)){ # for each locus
	j = union(genowide[,cols[i]], genowide[,cols[i]+1])
	j = j[!is.na(j)]
...
# max indivs per pop
numind = table(genowide$SiteNum)
maxind = max(numind)
	
	# Write to file
	name = paste("Aclarkii_", Sys.Date(), "_IBD", ".txt", sep="")
	file = file(name)
	open(file, "w")
	...
	for(j in 1:length(pops)){
		...
		if(length(k)>0){
			# collapse locus alleles with ""
			out = paste(rep(pops[j], length(k)), " ", sep="") # name each indiv according to population name
			out = cbind(out, paste(genowide[k,cols[1]], genowide[k,(cols[1]+1)], sep=",")) # format the genotypes for locus 1 with commas between alleles
			for(c in cols[2:length(cols)]){ # format genotypes for other loci
				out = cbind(out, paste(genowide[k,c], genowide[k,(c+1)], sep=","))
			}
			out[out==" NA NA"] = "0,0"
