...
for(i in 1:length(priors$names)){
	if(is.na(priors$De[i])){
		...
		De = D*nen
		priors$sigma[i] = sqrt(1/(4*priors$D[i]*priors$nen[i]*priors$m[i]))
	} else {
		De = priors$De[i]	
		priors$sigma[i] = sqrt(1/(4*priors$De[i]*priors$m[i]))
	}
	m = rnorm(len, mean = priors$m[i], sd = priors$mse[i])
	sigma = sqrt(1/(4*De*m))
...
par(mfrow=c(3,2))
numbreaks = 100
breaks = seq(0, 3, length.out=numbreaks)
for(i in 1:6){
	...
	print(paste(priors$names[i], "  Point:", round(priors$sigma[i], digits=1), "  Mean:", round(mean(temp, na.rm=T), digits=1), "  Median:", round(median(temp, na.rm=T), digits=1), "  L95:", round(quantile(temp, 0.025, na.rm=T), digits=1), "  U95:", round(quantile(temp, 0.975, na.rm=T), digits=1)))
}
