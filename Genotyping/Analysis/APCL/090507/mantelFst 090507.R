...
lines(cebu_geo[1:100][!is.na(cebu_geo[1:100])], l$fitted.values, col="black")
...
lines(leyte_geo[1:64][!is.na(leyte_geo[1:64])], l$fitted.values, col="black")
...
#	m = rnorm(len, mean=3.523e-05, sd = 1.412e-5) # slope of IBD using Cebu & Leyte together (neg Fsts remain), removed APCL285,286 and fixed APCL250
...
#	D = runif(len, 0.17, 252) # spp density from He of Neighborhoods (mu = 5e-4, 24000km, He=0.615, stepwise model), to Cebu-Leyte adult density
...
plot(-10,-10, xlim=c(-5,1.1*max(D)), ylim=c(0, 1.5/(max(D)-min(D))), ylab="Density", main="Effective density", xlab="Adults per km", bty="L")
rect(min(D), 0, max(D), 1/(max(D)-min(D)), col="grey80", border="NA")
...
plot(x,y,type="l", col=NA, bty="L", main="Slope", xlab="b", ylab="Density")
polygon(x,y, col="grey80", border=NA)
abline(v=1)#plot(density(m), xlab = "Slope", main = "Isolation by Distance Relationship")
