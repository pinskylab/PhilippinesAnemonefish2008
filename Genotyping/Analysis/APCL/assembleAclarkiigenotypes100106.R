...
###############################################
## Output to MLNE2: All samples (TopTwo on anemone >= 8cm vs. lower quartile, some pops as focal, certain others as source)

# sets of 1 w/ surrounding two as source
focalpops = list(b=8, c=9, d=10, e=11, f=13, g=14, h=15, i=16, j=19, k=20, l=22, m=23, n=24, o=25)

sourcepops = list(b=c(7,9), c=c(8,10), d=c(9,11), e=c(10,13), f=c(11,14), g=c(13,15), h=c(14,16), i=c(15,17), j=c(18,20), k=c(19,22), l=c(20,23), m=c(22,24), n=c(23,25), o=c(24,27))

prefix="TopTwoSurrTwo"
...
for(rep in 1:length(focalpops)){
	...
	# For source pop
	cols = grep("A1.", names(genowide))
	a = intersect(which(genowide$AdJuv == "Ad"), source_sites)
	print(paste("Pops ", paste(focal, collapse=","), ": Num Source: ", length(a), sep=""))
	for(i in cols){ # for each locus
		thesealleles = sort(c(genowide[a,i], genowide[a,i+1]))
		allalleles = alleles[[match(i,cols)]]
...
loci = unlist(strsplit(names(genowide)[cols], "A1.", fixed=T))[seq(2,26, by=2)]
	...
	# Write to file
	ldnename = paste("Aclarkii_", Sys.Date(), "_LDNE", ".gen", sep="")
	ldnefile = file(ldnename)
	open(ldnefile, "w")

	# Header
	cat(paste("Title line: Aclarkii all pops", sep=""), file= ldnefile, sep=",", append=F)
	cat("\n", file= ldnefile, append=T)
	cat(paste(loci, collapse="\n"), file= ldnefile, append=T)
	cat("\n", file= ldnefile, append=T) 
...
###############################################
## IBD web-service: All samples
## Also includes geographic distance

setwd("/Users/mpinsky/Documents/Stanford/Philippines/2008/Genotyping/Genotypes")
geno = read.csv("Aclarkii_genotypes_2009-03-13.csv", row.names=1)
geo = read.csv("../Analysis/APCL/090507/Aclarkii_2009-05-14 geo.csv", row.names=1)
source("../Analysis/matchall.R")	
...
# Write out each region to file
regions = c("Cebu", "Leyte")
for(r in 1:length(regions)){
	#pops = sort(unique(genowide$SiteNum)) # all populations
	pops = sort(unique(genowide$SiteNum[genowide$Region==regions[r]]))
	cols = grep("A1.", names(genowide))
	loci = unlist(strsplit(names(genowide)[cols], "A1.", fixed=T))[seq(2,26, by=2)]

	# max num alleles per locus in this region
	numall = numeric(13)
	alleles = vector("list", 13)
	for(i in 1:length(cols)){ # for each locus
		j = union(genowide[genowide$Region==regions[r],cols[i]], genowide[genowide$Region==regions[r],cols[i]+1])
		j = j[!is.na(j)]
		alleles[i] = list(sort(j))
		numall[i] = length(alleles[[i]])
	}
	maxall = max(numall)

	# max indivs per pop
	numind = table(genowide$SiteNum[genowide$Region==regions[r]])
	maxind = max(numind)
	
	# map allele sizes to allele numbers starting at one
	genowidemapped = genowide
	for(i in 1:length(cols)){
		newalls = 1:length(alleles[[i]])
		newgenos = rep(NA, length(genowide[,cols[i]]))
		for(j in 1:length(newalls)){
			k = genowide[,cols[i]]==alleles[[i]][j]
			newgenos[k] = newalls[j]	
		}
		genowidemapped[,cols[i]] = newgenos
		newgenos = rep(NA, length(genowide[,cols[i]+1]))
		for(j in 1:length(newalls)){
			k = genowide[,cols[i]+1]==alleles[[i]][j]
			newgenos[k] = newalls[j]	
		}
		genowidemapped[,cols[i]+1] = newgenos
	}
	
	# Open file for writing
	name = paste("Aclarkii_", regions[r], "_", Sys.Date(), "_IBD", ".txt", sep="")
	file = file(name)
	open(file, "w")

	# Header
	cat(paste(length(loci), " LOCI ", maxall, " ALLELES ", maxind, " INDIVIDUALS", sep=""), file= file, sep=",", append=F)
	cat("\n", file= file, append=T)

	for(j in 1:length(pops)){
		k = which(genowidemapped$SiteNum==pops[j])
		if(length(k)>0){
			# collapse locus alleles with ""
			out = rep(pops[j], length(k)) # name each indiv according to population name
			out = cbind(out, paste(genowidemapped[k,cols[1]], genowidemapped[k,(cols[1]+1)], sep=",")) # format the genotypes for locus 1 with commas between alleles
			for(c in cols[2:length(cols)]){ # format genotypes for other loci
				out = cbind(out, paste(genowidemapped[k,c], genowidemapped[k,(c+1)], sep=","))
			}
			out[out=="NA,NA"] = "0,0"
			for(c in 1:dim(out)[1]){ # write out to file with spaces between loci
				cat(paste(out[c,], collapse=" "), file=file, append=T)
				cat("\n", file= file, append=T)
			}	
		}
	}
	cat(paste("0 ", paste(rep("0,0", length(cols)), collapse=" "), "\n", sep=""), file=file, append=T)

	# write out geographic distances
	cat("GEOGRAPHIC_DISTANCE\n", file= file, append=T)

	k = match(pops, as.numeric(rownames(geo)))

	for(i in 1:(length(k)-1)){
		for(j in (i+1):length(k)){
			cat(paste(pops[i], pops[j], geo[k[j], k[i]], "\n", sep=" "), file=file, append=T)
		}
	}
	close(file)	
}
...
###############################################
## Arlequin jackknife over loci or drop two error rate loci: All samples

setwd("/Users/mpinsky/Documents/Stanford/Philippines/2008/Genotyping")
geno = read.csv("Genotypes/Aclarkii_genotypes_2009-03-13.csv", row.names=1)
source("Analysis/matchall.R")	
...
regions = c("Cebu", "Leyte")
drop = list(a = "AC1359", b="AC1578", c="AC915", d="ACH_B6", e="ACH_C1", f="ACH_D1", g="APR_Cf29", h="APR_Cf8", i="APY_45", j="APY_65", k= "NNG_004", l="NNG_012", m="NNG_028", n=c("APR_Cf29", "APY_65"), o = character(0))
...
for(r in 1:length(regions)){
	...
	for(d in 1:length(drop)){
		if(length(drop[[d]])>0){
			theseloci = loci[-matchall(drop[[d]], loci)]
		} else {
			theseloci = loci
		}
		thesecols = matchall(paste("A1.", theseloci, sep=""), names(genowide)) # columsn for the loci to output in this round
			
		# Open file for writing
		name = paste("Aclarkii_", regions[r], "_no", paste(drop[[d]], collapse=","), "_", Sys.Date(), ".arp", sep="")
		outfiles = c(outfiles, name)
		name = paste("Datafiles/Arlequin/", name, sep="")
		file = file(name)
		open(file, "w")

		# Header
		cat("[Profile]\n", file= file, append=T)
		cat(paste("\tTitle=\"A. clarkii CebuLeyte no ", paste(drop[[d]], collapse=" or "), "\"\n", sep=""), file= file, append=T)
...
# write out an Arlequin batch file
	name = paste("Datafiles/Arlequin/Aclarkii_jackknife_", Sys.Date(), ".arb", sep="")
	file = file(name)
	open(file, "w")
