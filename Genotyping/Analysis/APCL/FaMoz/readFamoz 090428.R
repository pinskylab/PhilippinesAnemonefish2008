...
famoz = readLines("090428/Famoz 090428.txt", n=-1)
ids=read.table("Aclarkii_create_2009-04-24noAC1359-FAMOZ-ID TABLE.txt", sep="\t", header=TRUE)
...
kids2 = subset(kids, select=c("RAW.ID", "FAMOZ.ID", "p1", "p2", "p3", "p4", "p1lod", "p2lod", "p3lod", "p4lod", "pp11", "pp12", "pp1lod"))


# erase pairs with lod < threshold 
i = kids2$pp1lod < threshpair
	kids2$pp11[i] = ""
	kids2$pp12[i] = ""
	kids2$pp1lod[i] = 0

# erase pair if single parent lods are < threshold for either parent
i = which(kids2$pp1lod>threshpair) 
for(j in i){
	par1 = kids2$pp11[j]
	par2 = kids2$pp12[j]
	k = which(c(kids2$p1[j],kids2$p2[j],kids2$p3[j],kids2$p4[j])==par1)
	if(length(k)==0){ print(paste("Didn't find a single parent match for pair", j))}
	if(kids2[[paste("p",k,"lod",sep="")]][j] < threshone){ # is parent1 < threshone?
		kids2$pp11[j] = "" # if so, delete this pair
		kids2$pp12[j] = ""
		kids2$pp1lod[j] = 0
	}
	k = which(c(kids2$p1[j],kids2$p2[j],kids2$p3[j],kids2$p4[j])==par2)
	if(length(k)==0){ print(paste("Didn't find a single parent match for pair", j))}
	if(kids2[[paste("p",k,"lod",sep="")]][j] < threshone){ # is parent2 < threshone?
		kids2$pp11[j] = "" # if so, delete this pair
		kids2$pp12[j] = ""
		kids2$pp1lod[j] = 0
	}
...
# erase single parent if < threshold
...
# drop the remaining single parents
kids2 = subset(kids2, select=c(-p2, -p3, -p4, -p2lod, -p3lod, -p4lod))
...
i = length(unique(kids2$RAW.ID[kids2$p1lod>0]))
print(paste("Number of unique kids assigned a single parent:", i))
i = length(unique(kids2$p1[kids2$p1lod>0]))
print(paste("Number of unique single parents assigned:", i))
i = sum(kids2$pp1lod>0)
print(paste("Number of parents pairs:", i))

# How many unique single parents? How many kids assigned to each parent?
i= kids2$p1[kids2$p1lod>0]
i = aggregate(i, by=list(i), length)
summary(i[,2])
j = hist(i[,2], breaks=c(.5, 1.5, 2.5, 3.5, 4.5))
j$counts
...
par(mfrow=c(1,2))
mean(c(kids2$dist1km,kids2$dist2km), na.rm=T)
...
## Graph mean dispersal dist as function of LODthreshold (single parents only)
t = seq(threshone, max(as.numeric(kids2$p1lod), na.rm=T), by=0.2)

y = numeric(length(t))
n = numeric(length(t))
for(i in 1:length(t)){
	y[i] = mean(kids2$dist1km[kids2$p1lod>=t[i]])
	n[i] = length(kids2$dist1km[kids2$p1lod>=t[i]])
}
par(mfrow=c(1,2))
plot(t,y, ylim=c(0, max(y)), type="b", xlab="Single parent LOD threshold", ylab="Distance between kid and parent", main="Dispersal")
plot(t,n, ylim=c(0, max(y)), type="b", xlab="Single parent LOD threshold", ylab="Number of kids assigned a parent", main="Number of assignments")
