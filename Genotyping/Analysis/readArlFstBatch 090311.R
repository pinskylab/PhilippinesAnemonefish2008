...
readArlFstBatch <- function (filename) {
	...
	fin = offset
	while(line[fin] != "" & fin < length(line)){ fin = fin+1 } # fine the end
	fin = fin-1

	fsts = vector("list", (length(line)-offset)) # set up list to hold fst matrices
	names = vector("list", (length(line)-offset))
