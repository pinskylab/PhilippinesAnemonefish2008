...
genowide=read.csv("PRBI_genowide_2009-11-12.csv", row.names=1)
...
# Format for GenAlEx 6
	...
	# Write to file
	genalexfile = file(paste("../Datafiles/GenAlEx/PRBI_GenAlEx_", Sys.Date(), ".csv", sep=""))
	open(genalexfile, "w")

	# Header: num loci, num samples, num pops, # indiv in each pop, # regions, # indivs in each region
	outline = c(16, dim(genowide)[1], length(unique(genowide$SiteNum)), table(genowide$SiteNum), length(unique(genowide$Region)), table(genowide$Region))
	...
	i = grep("A1[[:punct:]]", names(genowide))
...
# Output for create w/ Adult-Juvenile (conversion to FaMoz and Cervus)
	...
	# add Ad/juv info: Adults are top half, juvs are bottom half
	cutoff = median(genowide$Size, na.rm=T)
	genowide$AdJuv[genowide$Size > cutoff] = 1
	genowide$AdJuv[genowide$Size < cutoff] = 0

	
	# Write to file
	file = file(paste("../Datafiles/CREATE/PRBI_create_cut", cutoff, "_", Sys.Date(), ".csv", sep=""))
	open(file, "w")
