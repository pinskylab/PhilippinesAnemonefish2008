...
## Optional Cebu and Leyte APCL
	quartz(width=7, height=7)
	# pdf(width=7, height=7, file=paste('Figures/PRBI IBD wout19 PPT ', Sys.Date(), '.pdf', sep=''))
	# pdf(width=7, height=7, file=paste('Figures/APCL IBD PPT ', Sys.Date(), '.pdf', sep=''))
	# pdf(width=7, height=7, file=paste('Figures/APCL IBD wPRBIexpectation PPT ', Sys.Date(), '.pdf', sep=''))
	# pdf(width=7, height=7, file=paste('Figures/PRBI IBD wout19 wAPCL PPT ', Sys.Date(), '.pdf', sep=''))
	col = "white"
	...
	i = !(colnames(fstlin) %in% 'X19')
	j = !(colnames(geo) %in% 'X19')
	xlims = range(c(geo[j,j], apclgeo), na.rm=T)
	ylims = range(c(fstlin[i,i], apclfst), na.rm=T)
	
	plot(-100,-100, xlab="Geographic Distance (km)", ylab="Fst/(1-Fst)", main="PRBI", ylim=ylims, xlim = xlims, pch = 20, lwd=2, col=col.dot, xaxt='n', yaxt='n')
	axis(1, col=col, lwd=3)
	axis(2, col=col, lwd=3)
	
	# Cebu
	points(as.dist(apclcebu_geo), as.dist(apclcebu_fst), pch = 20, lwd=2, col=col.cebu, cex=0.7)
		# Regression line
		x = as.dist(apclcebu_geo)[1:length(as.dist(apclcebu_geo))]
		y = as.dist(apclcebu_fst)[1:length(as.dist(apclcebu_fst))]
		mod1=sma(y ~ x)
		k = order(x)
		lines(x[k], mod1$coef[[1]][1,1] + x[k]*mod1$coef[[1]][2,1], lty=1, lwd=3, col=col.cebu)

	# Leyte
	points(as.dist(apclleyte_geo), as.dist(apclleyte_fst), pch = 20, lwd=2, col=col.leyte, cex=0.7)
		# Regression line
		x = as.dist(apclleyte_geo)[1:length(as.dist(apclleyte_geo))]
		y = as.dist(apclleyte_fst)[1:length(as.dist(apclleyte_fst))]
		mod2=sma(y ~ x)
		k = order(x)
		lines(x[k], mod2$coef[[1]][1,1] + x[k]*mod2$coef[[1]][2,1], lty=1, lwd=3, col=col.leyte)
	
	# Premnas	
	points(as.dist(geo[j,j]), as.dist(fstlin[i,i]), pch = 20, lwd=2, col=col.dot)
		...
		mod3=sma(y ~ x)
		k = order(x)
		lines(x[k], mod3$coef[[1]][1,1] + x[k]*mod3$coef[[1]][2,1], lty=1, lwd=5, col=col.dot)

	# Premnas expected slope
		abline(-.01, 0.0002883335, col=col.dot, lwd=5)

	legend('topleft', pch=20, col=c(col.cebu, col.leyte, col.dot), legend=c('A. clarkii Cebu', 'A. clarkii Leyte', 'P. biaculeatus'), cex=0.6)
