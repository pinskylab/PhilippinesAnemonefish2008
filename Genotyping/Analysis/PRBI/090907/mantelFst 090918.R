...
setwd("/Users/mpinsky/Documents/Stanford/Philippines/2008/Genotyping/Analysis/PRBI/090907/")

## Read in data

#fst = read.csv("Arlequin/PRBI_2009-09-07_fsts_no12.csv", row.names=1) # Arlequin fsts (includes ACH_B9)
fst = read.csv("../090907_noACH_B9/Genepop/PRBI_2009-09-07_fsts_genepop_no12.csv", row.names=1) # Genepop w/out ACH_B9
geo = read.csv("Arlequin/PRBI_geo_no12_090918.csv", row.names=1) 
...
cebu_fst = as.matrix(fstlin[1:8,1:8])
cebu_geo = as.matrix(geo[1:8,1:8])

leyte_fst = as.matrix(fstlin[9:10,9:10])
...
plot(x$geo, x$fst, col=c("red", "blue")[as.numeric(x$spp)], pch=16)
abline(lm(x$fst[x$spp=="prbi"]~x$geo[x$spp=="prbi"]),col="blue")
abline(lm(x$fst[x$spp=="apcl"]~x$geo[x$spp=="apcl"]),col="red")
