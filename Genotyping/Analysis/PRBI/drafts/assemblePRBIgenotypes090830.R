...
afg58 = read.table("AFG58-59_PRBI_MLP Genotypes 090828.txt", header = T, sep ="\t", na.string="?")
...
afg51_57.mlp = read.table("AFG51-57_PRBI_MLP Genotypes 090730.txt", header = T, sep ="\t", na.string="?")
...
which(duplicated(paste(afg15$Sample, afg15$Marker))) # yes
...
# Check for allele dropout and choose genotype with more alleles
...
for(i in err){
	...
	for(j in 1:length(checkone)){ # for each genotype
		het[j] = geno[i,checkone[j]] != geno[i,checktwo[j]]
	}
	if(sum(het)==1){ # if only one het, rest are hom
	...
	if(sum(het)>1){ # if > one het, check for equality first
		matchone = all(geno[i,checkone[het]] == geno[i,checkone[which(het)[1]]]) # check for equality
		matchtwo = all(geno[i,checktwo[het]] == geno[i,checktwo[which(het)[1]]]) # check for equality
		if(matchone & matchtwo){
			geno$A1consens[i] = geno[i,checkone[het][1]]
			geno$A2consens[i] = geno[i,checktwo[het][1]]
			print(paste("Fixed", geno$Sample[i], geno$Marker[i]))
		} else {
			print(paste("Hets don't match for", geno$Sample[i], geno$Marker[i]))
		}
	}	
...
hom = cbind(hom,round((hom[,2]/(hom[,1]+hom[,2]))*100,1))
hom = hom[order(rownames(hom)),]
colnames(hom) = c("Hets", "Homs", "%Hom")
hom

# Remove unverified homozygotes for ACH_B9, NNG_004, and NNG_007
i = geno$hom & geno$numgenos < 2 & (geno$Marker == "ACH_B9" | geno$Marker == "NNG_004" | geno$Marker == "NNG_007")
sum(i)
table(i, geno$Marker)
geno$A1consens[i] = NA
geno$A2consens[i] = NA
...
setwd("/Users/mpinsky/Documents/Stanford/Philippines/2008/Genotyping/Genotypes")
geno = read.csv("PRBI_genotypes_2009-08-30.csv", row.names=1)
drop = c("ACH_D8", "APR_Cf8", "APY_2", "APY_44", "APY_45", "APY_65") # loci to remove
for(i in drop){
	j = which(geno$Marker==i)
	geno = geno[-j,]	
}
