...
cebu_geo = as.matrix(geo[3:9,3:9]) # geo is missing pop 2 right now
...
plot(as.dist(geo), as.dist(fstlin), xlab="Geographic Distance (km)", ylab="Fst/(1-Fst)", main="Isolation-by-distance", ylim=ylim, xlim = xlims, pch = 20, lwd=2, col=cols)
x = as.dist(geo)
y = as.dist(fstlin)
l=(lm(y ~ x))
...
plot(as.dist(apclcebu_fst[1:cebu, 1:cebu]), as.dist(cebu_fst), xlab="APCL Fst/(1-Fst)", ylab="PRBI Fst/(1-Fst)", pch=16, main="APCL vs. PRBI Genetic Distance")
mantel(apclcebu_fst[1:cebu, 1:cebu], cebu_fst)

# PRBI vs. APCL fsts: Mantel comparisons (all data)

plot(as.dist(apclfst), as.dist(fst), xlab="APCL Fst/(1-Fst)", ylab="PRBI Fst/(1-Fst)", pch=16, main="APCL vs. PRBI Genetic Distance")
mantel(apclfst, fst)
